...
router.get("/users", isAdmin, (req, res) => {
  User.find()
    ...
    .catch((error) => {
      res.flash(
        "error_message",
        "Something gone wrong when loading users!\n" + error
      );
      res.redirect("/admin");
    });
});
...
router.get("/categories", isAdmin, (req, res) => {
  Category.find()
    ...
    .catch((error) => {
      res.flash(
        "error_message",
        "Oh dog, something gone wrong on categories\n" + error
      );
      res.redirect("/admin");
    });
});
...
router.get("/categories/edit/:id", isAdmin, (req, res) => {
  Category.findOne({ _id: req.params.id })
    ...
    .catch((error) => {
      req.flash(
        "error_message",
        "Oh bullocks! this category doesn't exist!\n" + error
      );
      res.redirect("/admin/categories");
    });
});
...
router.post("/categories/edit", isAdmin, (req, res) => {
  Category.findOne({ _id: req.body.id })
    .then((category) => {
      ...
      category
        ...
        .catch((error) => {
          req.flash(
            "error_message",
            "Come on dawg, something is wrong when saving!\n" + error
          );
          res.redirect("/admin/categories");
        });
    })
    .catch((error) => {
      req.flash(
        "error_message",
        "Come on dawg, something is wrong with update!\n" + error
      );
      res.redirect("/admin/categories");
    });
});
...
router.get("/posts", isAdmin, (req, res) => {
  Post.find()
    ...
    .catch((error) => {
      res.flash(
        "error_message",
        "Oh sheep, something gone wrong on posts\n" + error
      );
      res.redirect("/admin");
    });
});
...
router.get("/posts/edit/:id", isAdmin, (req, res) => {
  Post.findOne({ _id: req.params.id })
    .then((post) => {
      Category.find()
        ...
        .catch((error) => {
          req.flash(
            "error_message",
            "NOOOOO! Something gone wrong on category listing!\n" + error
          );
          res.redirect("/admin/posts");
        });
    })
    .catch((error) => {
      req.flash(
        "error_message",
        "Oh bullocks! this post doesn't exist!\n" + error
      );
      res.redirect("/admin/posts");
    });
});
